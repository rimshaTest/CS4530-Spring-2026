...
In this activity, you will understanding the server by reading the woefully uncommented code and interacting with it with HTTP requests. One way to do this is with [Postman] XXX LINK, but you can also just write cURL commands if you want. You’ll also try your hand at using [Zod](https://zod.dev/api?id=objects) to describe the desired shape of input. Unlike TypeScript types, Zod validators can actually check whether inputs have the right shape.
