...
This activity involves understanding the server by reading the woefully uncommented code and interacting with it with HTTP requests. One way to do this is with [Postman]({{site.baseurl}}{% link tutorials/week1-api-requests-postman.md %}). You’ll also try your hand at using [Zod](https://zod.dev/api?id=objects) to describe the desired shape of input in a way that — unlike TypeScript types — can actually check whether inputs have the right shape.
